import { useState, useEffect } from 'react'
import { DashboardBarChart, DashboardDonutChart, DashboardHeader, MetricCard, ProgressLine, SectionPanel, statusClass } from './DashboardToolkit'
import { dashboardStatusTone } from './roleDashboardLayouts'
import { getDashboardData } from '../../services/api'

import DashboardSkeleton from './DashboardSkeleton'

export default function DashboardDinkes() {
  const [filter, setFilter] = useState('')
  const [dashboardData, setDashboardData] = useState(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setLoading(true)
    getDashboardData('dinkes', filter)
      .then((res) => {
        setDashboardData(res)
        if (res?.selectedFilter && res.selectedFilter !== filter) {
          setFilter(res.selectedFilter)
        }
        setLoading(false)
      })
      .catch((err) => {
        console.error('Failed to load dashboard:', err)
        setLoading(false)
      })
  }, [filter])

  if (loading || !dashboardData) {
    return <DashboardSkeleton />
  }

  const current = dashboardData.data
  const filters = dashboardData.filters
  const selectedFilter = dashboardData.selectedFilter || filter || filters?.[0]?.value || ''

  return (
    <div className="space-y-6">
      <DashboardHeader
        title="Dashboard Dinkes"
        description={current.summary}
        filters={filters}
        value={selectedFilter}
        onChange={setFilter}
      />

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {current.cards.map((item) => (
          <MetricCard key={item.label} {...item} />
        ))}
      </div>

      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <SectionPanel title="Ringkasan Wilayah" subtitle="Sorotan cepat yang mewakili seluruh puskesmas di wilayah kerja.">
          <div className="space-y-3">
            {current.mapSummary.map((item) => (
              <div key={item.label} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                <div className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">{item.label}</div>
                <div className="mt-2 text-lg font-bold text-slate-900 dark:text-white">{item.value}</div>
                <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{item.note}</p>
              </div>
            ))}
          </div>
          <div className="mt-6">
            <div className="mb-4 font-semibold text-slate-900 dark:text-white">{current.donut.title}</div>
            <div className="h-[280px]">
              <DashboardDonutChart labels={current.donut.labels} series={current.donut.series} />
            </div>
          </div>
        </SectionPanel>

        <SectionPanel title={current.trend.title} subtitle="Tren ini mencakup gabungan seluruh puskesmas di wilayah yang dipantau.">
          <div className="h-[420px]">
            <DashboardBarChart categories={current.trend.categories} series={current.trend.series} />
          </div>
        </SectionPanel>
      </div>

      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <SectionPanel title={current.rankings.title} subtitle="Perbandingan performa antar puskesmas untuk kebutuhan pembinaan dan evaluasi.">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] border-collapse text-left">
              <thead>
                <tr className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 dark:bg-slate-900/50 dark:text-slate-400">
                  <th className="p-4 font-bold">Puskesmas</th>
                  <th className="p-4 font-bold">IKS</th>
                  <th className="p-4 font-bold">Coverage</th>
                  <th className="p-4 font-bold">Arah</th>
                  <th className="p-4 font-bold">Status</th>
                </tr>
              </thead>
              <tbody className="text-sm font-medium">
                {current.rankings.rows.map((row) => (
                  <tr key={row.unit} className="border-b border-slate-100 dark:border-slate-800">
                    <td className="p-4 text-slate-900 dark:text-white">{row.unit}</td>
                    <td className="p-4 text-slate-600 dark:text-slate-300">{row.iks}</td>
                    <td className="p-4 text-slate-600 dark:text-slate-300">{row.coverage}</td>
                    <td className="p-4 text-slate-600 dark:text-slate-300">{row.trend}</td>
                    <td className="p-4">
                      <span className={`rounded-xl px-3 py-1.5 text-xs font-bold ${statusClass(dashboardStatusTone(row.status))}`}>
                        {row.status}
                      </span>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </SectionPanel>

        <SectionPanel title="Fokus Indikator Wilayah" subtitle="Membantu membaca indikator dominan bermasalah di level dinas.">
          <div className="space-y-4">
            {current.focusIndicators.map((item) => (
              <div key={item.label} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="font-semibold text-slate-900 dark:text-white">{item.label}</div>
                    <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">{item.note}</div>
                  </div>
                  <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusClass(dashboardStatusTone(item.status))}`}>
                    {item.value}
                  </span>
                </div>
                <div className="mt-3">
                  <ProgressLine value={item.progress} />
                </div>
              </div>
            ))}
          </div>
        </SectionPanel>
      </div>
    </div>
  )
}

